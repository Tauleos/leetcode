...
var displayTable = function (orders) {
	...
	for (let order of orders) {
		names.add(order[2]);
		let foods = foodsMap.get(order[1]) || new Map();
		foods.set(order[2], (foods.get(order[2]) || 0) + 1);
		foodsMap.set(order[1], foods);
	}
	let tableHeader = Array.from(names).sort();
	// console.log(table);
	// tableHeader.unshift('Table');
	// console.log(table);
	let table = [];
	for (let [t, food] of foodsMap.entries()) {
		let temp = [t];
		for (let i of tableHeader) {
			temp.push(food.has(i) ? '' + food.get(i) : '0');
		}
		table.push(temp);
	}
	tableHeader.unshift('Table');
	table.sort((a, b) => (Number(a[0]) > Number(b[0]) ? 1 : -1));
	// console.log(table);
