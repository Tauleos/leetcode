 ...
 * 更有钱。另给你一个整数数组 quiet ，其中 quiet[i] 是 person i 的安静值。richer 中所给出的数据 逻辑自恰（也就是说，在
...
var loudAndRich = function (richer, quiet) {
	...
	let relative = new Array(n);
	for (let i = 0; i < n; i++) {
		relative[i] = [];
	}
	for (let r of richer) {
		relative[r[1]].push(r[0]);
	}
	...
	const dfs = (person) => {
		...
		// console.log('person', person, res);
		//首先设置比我有钱的安静人是我自己
		res[person] = person;
		for (let i of relative[person]) {
			dfs(i);
			//然后遍历搜索比我有钱的人y1到yn，找到他们各自的比他们有钱的安静人
			if (quiet[res[person]] > quiet[res[i]]) {
				//如果Yn的有钱安静人比我的有钱安静人还安静，那就替换过去
				res[person] = res[i];
			}
		}
	};
