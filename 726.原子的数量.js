...
var countOfAtoms = function (formula) {
	...
	while (i < n) {
		const getAtom = () => {
			let t = formula[i++];
			while (i < n) {
				if (/[a-z]/g.test(formula[i])) {
					// console.log('aaa', t, formula[i]);
					t += formula[i++];
					// console.log('bbb', t, formula[i]);
				}
				break;
			}
			return t;
		...
		if (formula[i] === '(') {
			...
			const top = stack[0];
			for (let [atom, v] of pop.entries()) {
				top.set(atom, (top.get(atom) || 0) + v * num);
			}
		} else {
			let atom = getAtom();
			let num = getNum();
